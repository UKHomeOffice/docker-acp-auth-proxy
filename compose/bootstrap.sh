...
CLIENT_SECRET=$(curl -s -X GET \
  -H "Authorization: bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  "http://${KEYCLOAK_INSTANCE}/auth/admin/realms/${REALM}/clients/${CLIENT_ID}/client-secret" | jq -r '.value'
)

# for use in the auth proxy
export KEYCLOAK_CLIENT_SECRET="${CLIENT_SECRET}"
export SESSION_SECRET="${SESSION_SECRET:-$(tr -dc A-Za-z0-9 </dev/urandom | head -c 32; echo '')}" # generate a random session secret 32 chars long (if there isn't already one set)

# jq isn't needed anymore so remove from the mounted volume
rm "${COMPOSE_DIR}/jq"

# use docker's default dns server as the resolver
